...
io.on('connection', (socket)=> {
    ...
    // Store the room that the default socket is connected to
    ...
    socket.on('JOIN', (roomName)=>{
        ...
        // Reject join request if room already has more than 1 conversation
        if (room && room.length > 1){
            io.to(socket.id).emit('ROOM_FULL', null)

            // Notify the room that someone tried to join
